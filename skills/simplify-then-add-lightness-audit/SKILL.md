...
| `Taskweft.HRR.*` (`multiplayer-fabric-taskweft/lib/taskweft/hrr/`) | Per-entity episodic memory for the RECTGTN HTN planner. Each persona owns its own episodes (source = "entity:{uuid}") — personas do not share bodies or memories. One SQLite file per zone holds all entities' namespaces. Integration: probe entity memory → inject top-k episodes into domain state → plan → store result. Cross-entity learning is opt-in only. Placement: alongside the zone process (not zone-backend/CRDB). Do not remove even with zero callers. |
